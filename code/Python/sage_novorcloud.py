...
for d in argv[1:]:
    ...
    novor = pd.read_csv(os.path.join(d, 'denovo.csv'))

    novor_seqs = []
    for seq in novor['De Novo Peptide']:
        novor_seqs.append(''.join(aa for aa in seq.replace('(N-term|T6n)', '').replace('(N-term|Tn)', '').replace('(T6K)','').replace('(TK)','').replace('(O)','').replace('(Pyro-Glu)','').replace('(Cam)','') if aa.isalpha() and aa.isupper()))
        
    novor = novor.assign(sequence=novor_seqs)
    novor['seq_len'] = novor['sequence'].str.len()

    merged = novor.merge(psms, left_on='Scan #', right_on='scan')
...
for i, row in merged.iterrows():
    distance = edit_distance(row['sequence'].replace('I', 'L'), row['bare_sequence'].replace('I', 'L'), transpositions=True)
    distances.append(distance)
    
    sage_aas = []
    novo_aas = []
    for i, aa in enumerate(row['bare_sequence']):
        sage_aas.append({'aa':aa2mass[aa], 'prefix':sum(aa2mass[aap] for aap in row['bare_sequence'][:i])})
    for i, aa in enumerate(row['sequence']):
        novo_aas.append({'aa':aa2mass[aa], 'prefix':sum(aa2mass[aap] for aap in row['sequence'][:i])})
        
    aa_match = [False] * len(row['sequence'])
    for i, aa in enumerate(novo_aas):
        if any(abs(aa['aa']-aa2['aa'])<0.1 and abs(aa['prefix']-aa2['prefix'])<0.5 for aa2 in sage_aas):
            aa_match[i] = True
    aa_matched.append(aa_match)
    
merged = merged.assign(distance=distances, aa_matched=aa_matched)
merged = merged[merged['distance'] != -1]

tagged_psms = merged[merged['tag'] > 0]
lf_psms = merged[merged['tag'] == 0]
if BEST_PSM:
    tagged_psms = tagged_psms.sort_values('Score', ascending=False).drop_duplicates(['peptide', 'charge'])
    lf_psms = lf_psms.sort_values('Score', ascending=False).drop_duplicates(['peptide', 'charge'])
...
for score in np.arange(100, 0, -0.1):
    filtered_lf = lf_psms[lf_psms['Score'] >= score]
    filtered_t6 = tagged_psms[tagged_psms['Score'] >= score]
    ...
    filtered_lf = intersect[intersect['Score_x'] >= score]
    filtered_t6 = intersect[intersect['Score_y'] >= score]
...
fig.set_size_inches(5,5)
axs = fig.subplot_mosaic([['a','a'],['b','c']], gridspec_kw={'height_ratios':[2,1], 'hspace':0.1})
